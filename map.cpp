...
char getMapBlock(char map_x, char map_y, char room)
{
  ...
  index_ptr = pgm_read_byte_near(map_room_data + (room * 10) + map_x);
  //index_ptr = pgm_read_byte_near(map_room_data + (room * 12) + map_x);
  
  //determine which pattern is being referenced
  //and return this value
  return pgm_read_byte_near (map_pattern_data + (index_ptr * 6) + map_y);
  //return pgm_read_byte_near (map_pattern_data + (index_ptr * 8) + map_y);
}

void drawMapRoom()
{ 
  gb.display.clear();//Clear display before drawing new room
  for (char y=0;y<6; y++)
  //for (char y=0;y<8; y++)
  {
    for (char x=0;x<10; x++)
...
void scrollMap(char direction)
{
  ...
  //gb.display.print((int)map_curr_room);
}
