...
void update_sound()
{
  ...
  {
  case SONG_PLAYING:
    return;//For now let's kill it before it tries to play the bagpipes
  case SONG_PLAYING_ONCE:
  case SONG_PLAYING_SFX:
    static long next_note_start_time = 0;
    current_time+=100;
    if (current_time >= next_note_start_time)
    //if (TV.millis() >= next_note_start_time)
    {
     ...
     //determine the current note duration
      ...
      next_note_start_time += 100+note_duration;//Going by 100 ms per frame
      //next_note_start_time = TV.millis()+note_duration;
	  
	  //play the note
  // gb.sound.playNote(42, note_duration, 0);
    gb.sound.playNote(pgm_read_word_near(melody + current_note), note_duration, 0);
...
{
  //gb.sound.stopPattern(1);
  //gb.sound.playPattern(myPattern,1);
  return;   //NOTE EARLY RETURN!!  Currently NOT playing any music.
